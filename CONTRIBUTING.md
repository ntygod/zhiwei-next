...
7. R2/R3 在 Draft 状态下使用新的独立 AI 上下文审查当前 HEAD，并留下批准记录。
8. 将 `independent-review` 更新为 `complete`，再把 PR 标为 Ready；Ready 事件会触发一轮绑定当前 HEAD 和最终合同的新 CI。
9. 满足门禁后由 AI 或自动工作流 squash merge。
10. 更新 Issue、状态和后续项，然后继续下一项工作。

R0/R1 不需要独立审查，完成作者自审后即可标为 Ready。没有人类回复时继续推进；人类明确评论或验收结果优先于自动队列。

## 风险与合并

- `R0/R1`：CI + 作者自审即可自主合并。
- `R2/R3`：需要回滚/恢复说明和当前 HEAD 的独立 AI 审查。
- 治理、CI、自动合并、安全和发布变化按 R3。
