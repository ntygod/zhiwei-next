...
async function loadInstalledModules() {
  ...
  if (codingManifest.name !== PI_PACKAGE_NAME || codingManifest.version !== PI_PACKAGE_VERSION) {
    throw new Error(
      `Unexpected coding-agent manifest: ${codingManifest.name}@${codingManifest.version}`,
    );
  }
  if (aiManifest.name !== "@earendil-works/pi-ai" || aiManifest.version !== PI_PACKAGE_VERSION) {
  ...
  return { codingDir, codingManifest, aiManifest, coding, faux };
}

async function captureSurface() {
  stage = "surface:inspect";
  const { codingDir, codingManifest, aiManifest, coding } = await loadInstalledModules();
  const rpcClientMethods = Object.getOwnPropertyNames(coding.RpcClient?.prototype ?? {})
    .filter((name) => name !== "constructor")
    .sort();
  const missingRpcClientMethods = SDK_RPC_PARITY_REQUIRED_RPC_CLIENT_METHODS.filter(
    (name) => !rpcClientMethods.includes(name),
  );
  const files = await Promise.all(
    SDK_RPC_PARITY_SURFACE_FILES.map(async (path) => {
      const bytes = await readFile(join(codingDir, path));
  ...
  const sourceByPath = Object.fromEntries(
    await Promise.all(
      SDK_RPC_PARITY_SURFACE_FILES.map(async (path) => [
        path,
        await readFile(join(codingDir, path), "utf8"),
      ]),
    ),
  );
  const packageExports =
    codingManifest.exports && typeof codingManifest.exports === "object"
      ? Object.keys(codingManifest.exports).sort()
      : [];
  const packageBin =
    codingManifest.bin && typeof codingManifest.bin === "object"
      ? Object.fromEntries(
          Object.entries(codingManifest.bin).sort(([left], [right]) => left.localeCompare(right)),
        )
      : codingManifest.bin;

  return {
    package: {
      name: codingManifest.name,
      version: codingManifest.version,
      dependencyPiAiVersion: aiManifest.version,
      main: codingManifest.main ?? null,
      packageExports,
      packageBin,
      expected: {
    ...
    rootExports: {
      exportNames: Object.keys(coding).sort(),
      runRpcModeType: typeof coding.runRpcMode,
      runRpcModeArity: typeof coding.runRpcMode === "function" ? coding.runRpcMode.length : null,
      rpcClientType: typeof coding.RpcClient,
    },
    rpcClient: {
      prototypeMethods: rpcClientMethods,
      requiredMethods: [...SDK_RPC_PARITY_REQUIRED_RPC_CLIENT_METHODS],
      missingRequiredMethods: missingRpcClientMethods,
    },
    files,
    structuredSignals: {
      rootIndexReexportsModes: /from\s+["']\.\/modes\/index\.js["']/.test(
        sourceByPath["dist/index.js"],
      ),
      rpcEntryForcesRpcMode: /["']--mode["']\s*,\s*["']rpc["']/.test(
        sourceByPath["dist/rpc-entry.js"],
      ),
      modesIndexExportsRunRpcMode: sourceByPath["dist/modes/index.js"].includes("runRpcMode"),
      modesIndexExportsRpcClient: sourceByPath["dist/modes/index.js"].includes("RpcClient"),
      ...
      rpcModeExposesStateAndMessages:
        sourceByPath["dist/modes/rpc/rpc-mode.js"].includes("get_state") &&
        sourceByPath["dist/modes/rpc/rpc-mode.js"].includes("get_messages"),
      jsonlUsesLfOnlyBuffering:
        sourceByPath["dist/modes/rpc/jsonl.js"].includes('indexOf("\\n")') &&
        !sourceByPath["dist/modes/rpc/jsonl.js"].includes("node:readline"),
    },
    sanitization: {
...
async function captureSdk() {
  ...
  for (const [name, value] of Object.entries({
    ...
    if (value === undefined) throw new Error(`Required SDK export is missing: ${name}`);
  }
  ...
  const preflight = [];
  stage = "sdk:prompt";
  await sdkSession.prompt(SDK_RPC_PARITY_PROMPT, {
    source: "interactive",
  ...
  return {
    ...
    lifecycleNotes: [
      ...
      {
        type: "dispose-host-boundary",
        mechanism: "session.dispose",
      },
    ],
  };
}
...
function sanitizeRpcState(data) {
  return {
    model: data?.model
      ? { provider: data.model.provider, id: data.model.id, api: data.model.api }
      : null,
...
function sanitizeRpcResponse(record, sequence) {
  ...
  if (record.success && record.command === "get_state") {
    output.state = sanitizeRpcState(record.data);
  }
  if (record.success && record.command === "get_available_models") {
    const models = record.data?.models ?? [];
    const targetModel = models.find(
      (model) =>
        model.provider === SDK_RPC_PARITY_PROVIDER_ID && model.id === SDK_RPC_PARITY_MODEL_ID,
    );
  ...
  if (record.success && record.command === "get_messages") {
    output.messages = (record.data?.messages ?? []).map((message, index) =>
...
async function captureRpc() {
  ...
  const exitPromise = new Promise((resolvePromise, reject) => {
    ...
    child.once("exit", (code, signal) => {
      ...
      if (waiters.size > 0) {
        rejectWaiters(
          new Error(
            `RPC Worker exited before ${waiters.size} awaited record(s): code=${code}, signal=${signal}.`,
          ),
        );
      }
  ...
  send({ id: SDK_RPC_PARITY_COMMAND_IDS.availableModels, type: "get_available_models" });
  const modelsResponse = await response(SDK_RPC_PARITY_COMMAND_IDS.availableModels);
  ...
  send({
    id: SDK_RPC_PARITY_COMMAND_IDS.setModel,
    type: "set_model",
    provider: SDK_RPC_PARITY_PROVIDER_ID,
    modelId: SDK_RPC_PARITY_MODEL_ID,
  });
  const setModelResponse = await response(SDK_RPC_PARITY_COMMAND_IDS.setModel);
  if (!setModelResponse.success) throw new Error(`set_model failed: ${setModelResponse.error}`);

  send({
    id: SDK_RPC_PARITY_COMMAND_IDS.setThinking,
    type: "set_thinking_level",
    level: "off",
  });
  const setThinkingResponse = await response(SDK_RPC_PARITY_COMMAND_IDS.setThinking);
  if (!setThinkingResponse.success) {
    throw new Error(`set_thinking_level failed: ${setThinkingResponse.error}`);
  }

  send({ id: SDK_RPC_PARITY_COMMAND_IDS.stateBefore, type: "get_state" });
  const stateBeforeRaw = await response(SDK_RPC_PARITY_COMMAND_IDS.stateBefore);
  const stateBefore = sanitizeRpcState(stateBeforeRaw.data);

  stage = "rpc:prompt";
  send({
    id: SDK_RPC_PARITY_COMMAND_IDS.prompt,
    type: "prompt",
    message: SDK_RPC_PARITY_PROMPT,
  });
  const promptResponseRaw = await response(SDK_RPC_PARITY_COMMAND_IDS.prompt);
  if (!promptResponseRaw.success) {
    throw new Error(`RPC prompt preflight failed: ${promptResponseRaw.error}`);
  }
  const promptResponseWireIndex = rawRecords.indexOf(promptResponseRaw);

  send({ id: SDK_RPC_PARITY_COMMAND_IDS.stateDuring, type: "get_state" });
  const stateDuringRaw = await response(SDK_RPC_PARITY_COMMAND_IDS.stateDuring);
  const stateDuring = sanitizeRpcState(stateDuringRaw.data);
  const stateDuringResponseWireIndex = rawRecords.indexOf(stateDuringRaw);

  const settledRaw = await waitFor(
    (record) => record.type === "agent_settled",
    "RPC agent_settled",
    30_000,
  );
  const settledWireIndex = rawRecords.indexOf(settledRaw);
  const agentStartWireIndex = rawRecords.findIndex((record) => record.type === "agent_start");

  stage = "rpc:final-queries";
  send({ id: SDK_RPC_PARITY_COMMAND_IDS.stateAfter, type: "get_state" });
  send({ id: SDK_RPC_PARITY_COMMAND_IDS.messagesAfter, type: "get_messages" });
  send({
    id: SDK_RPC_PARITY_COMMAND_IDS.lastTextAfter,
    type: "get_last_assistant_text",
  });
  const [stateAfterRaw, messagesAfterRaw, lastTextAfterRaw] = await Promise.all([
    response(SDK_RPC_PARITY_COMMAND_IDS.stateAfter),
    response(SDK_RPC_PARITY_COMMAND_IDS.messagesAfter),
    response(SDK_RPC_PARITY_COMMAND_IDS.lastTextAfter),
  ]);
  ...
  for (const record of responseRecords) {
    responseIdCounts.set(record.id, (responseIdCounts.get(record.id) ?? 0) + 1);
  }
  const responseIdsExactlyMatchCommands =
    responseRecords.length === commands.length &&
    commands.every((command) => responseIdCounts.get(command.id) === 1);
  ...
  return {
    commandLine: {
      ...
      automaticSkillDiscoveryDisabled: true,
      automaticTemplateDiscoveryDisabled: true,
      automaticThemeDiscoveryDisabled: true,
      automaticContextDiscoveryDisabled: true,
    },
    commands,
    responses: {
    ...
    ordering: {
      ...
      agentStartWireIndex,
      stateDuringResponseWireIndex,
      settledWireIndex,
      promptResponseBeforeAgentStart:
        promptResponseWireIndex >= 0 && agentStartWireIndex > promptResponseWireIndex,
      promptResponseBeforeDuringStateResponse:
        promptResponseWireIndex >= 0 && stateDuringResponseWireIndex > promptResponseWireIndex,
      stateDuringResponseBeforeSettled:
        stateDuringResponseWireIndex >= 0 && settledWireIndex > stateDuringResponseWireIndex,
    ...
    wireSemantics: {
      responseRecordsWithIds: responseRecords.every(
        (record) => typeof record.id === "string" && record.id.length > 0,
      ),
      responseIdsExactlyMatchCommands,
      runtimeEventsContainIds: rawRecords
    ...
    worker: {
      ...
      close: { code: close.code, signal: close.signal },
      exitBeforeClose:
...
async function run() {
  ...
  completed.surface = await captureSurface();
  ...
  const result = {
    ...
    contract: {
      ...
      prompt: SDK_RPC_PARITY_PROMPT,
      finalText: textSummary(SDK_RPC_PARITY_FINAL_TEXT),
      provider: SDK_RPC_PARITY_PROVIDER_ID,
      api: SDK_RPC_PARITY_API_ID,
      modelId: SDK_RPC_PARITY_MODEL_ID,
      responseId: SDK_RPC_PARITY_RESPONSE_ID,
      tokensPerSecond: SDK_RPC_PARITY_TOKENS_PER_SECOND,
      tokenSize: SDK_RPC_PARITY_TOKEN_SIZE,
      tools: [],
    },
    cases: completed,
    comparison: {
      surfaceAvailable:
        completed.surface.rootExports.runRpcModeType === "function" &&
        completed.surface.rootExports.rpcClientType === "function" &&
        completed.surface.rpcClient.missingRequiredMethods.length === 0,
      sdkProjection,
      rpcProjection,
      ...
      rpcPromptResponseIsAcceptanceBoundary:
        completed.rpc.ordering.promptResponseBeforeAgentStart &&
      ...
      sourcesRemainDistinct: {
        sdkPublic: true,
        sdkExtension: true,
        rpcCommandResponses: true,
        rpcRuntimeEvents: true,
        rpcProcessBoundaries: true,
      },
    },
    sanitization: {
      absolutePathsIncluded: false,
  ...
  const surfaceSignals = Object.values(completed.surface.structuredSignals);
  const requirements = [
    [completed.surface.rootExports.runRpcModeType === "function", "runRpcMode root export is missing."],
    [completed.surface.rootExports.rpcClientType === "function", "RpcClient root export is missing."],
    [completed.surface.rpcClient.missingRequiredMethods.length === 0, "RpcClient required methods drifted."],
    [completed.surface.files.length === SDK_RPC_PARITY_SURFACE_FILES.length, "RPC surface file set is incomplete."],
    [
      completed.surface.files.every(
        (file) => file.size > 0 && /^[0-9a-f]{64}$/.test(file.sha256),
      ),
      "RPC surface file metadata is invalid.",
    ],
    [surfaceSignals.every(Boolean), "Published RPC structured source signals drifted."],
    ...
    [
      completed.sdk.preflight.length === 1 && completed.sdk.preflight[0].success === true,
      "SDK preflight must succeed exactly once.",
    ],
    [
      completed.sdk.firstStreamingState?.isStreaming === true,
      "SDK did not expose a streaming state during message updates.",
    ],
    [
      completed.sdk.after.isIdle === true && completed.sdk.after.isStreaming === false,
      "SDK prompt did not settle at an idle boundary.",
    ],
    ...
    [
      completed.rpc.stateDuring.isStreaming === true,
    ...
    [
      completed.rpc.ordering.promptResponseBeforeAgentStart === true,
      "RPC Prompt response must precede agent_start.",
    ],
    [
      completed.rpc.ordering.promptResponseBeforeSettled === true,
      "RPC Prompt response must precede agent_settled.",
    ],
    [
      completed.rpc.ordering.stateDuringResponseBeforeSettled === true,
      "RPC running State response must precede agent_settled.",
    ],
    [
      completed.rpc.ordering.eventCountAfterPromptResponseBeforeSettled > 0,
      "RPC Prompt response did not precede continuing Runtime events.",
    ],
    [
      completed.rpc.wireSemantics.responseRecordsWithIds === true,
      "RPC response records must carry IDs.",
    ],
    [
      completed.rpc.wireSemantics.responseIdsExactlyMatchCommands === true,
      "RPC response IDs do not exactly match command IDs.",
    ],
    [
      completed.rpc.wireSemantics.runtimeEventsContainIds === false,
      "RPC Runtime events unexpectedly carry command IDs.",
    ],
    [
      completed.rpc.wireSemantics.messageUpdatesContainPartial === false,
      "RPC message_update leaked cumulative partial snapshots.",
    ],
    [
      completed.rpc.worker.exit.code === 0 && completed.rpc.worker.exit.signal === null,
      "RPC Worker exit boundary was not clean.",
    ],
    [
      completed.rpc.worker.close.code === 0 && completed.rpc.worker.close.signal === null,
      "RPC Worker close boundary was not clean.",
    ],
    ...
    [
      ...
      "RPC Worker left an unterminated stdout record.",
    ],
    [completed.rpc.worker.stderrPresent === false, "RPC Worker wrote unexpected stderr."],
    [
      completed.rpc.extensionEvidence.shutdown?.reason === "quit",
      "RPC Extension did not observe shutdown(reason=quit).",
    ],
    [
      completed.rpc.extensionEvidence.provider?.callCount === 1,
      "RPC Faux provider must consume exactly one response.",
    ],
    [
      completed.rpc.extensionEvidence.provider?.promptsSentToExternalProvider === 0,
      "RPC probe reached an external provider.",
    ],
