...
- 正式 open 只能使用模块内固定 migration source；pending migrations、history、user_version 与最终验证一次原子提交，公开 API 不得暴露 migration override。
...
- 文件数据库必须证明 WAL 与 `PRAGMA integrity_check=ok`；`:memory:` 的 journal mode 单独记录，不能冒充文件 WAL。exact-head 验证使用受支持的 Node.js 22.x line，记录实际 patch 并在每个新 HEAD 上重跑完整真实 `node:sqlite` 矩阵。
